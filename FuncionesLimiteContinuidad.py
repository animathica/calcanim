...
class Limite4_1 (ThreeDScene):
    ...
    def custom_method(self):
        ...
        cir1=Circle(radius=1, color=RED).move_to(3*UP + 3*RIGHT)
...
class ImgInversa(GraphScene):
    def construct(self):
        ...
        text_1b = TexMobject(r"f: A\subset\mathbb{R}^n \to \mathbb{R}^m").next_to(text_1a,DOWN)
        ...
        text_4 = TexMobject(r"\text{Buscamos } ", r"\{ \vec{x} \in A \vert f(\vec{x}) \in D \}").to_edge(UP)
        ...
        text_5b = TextMobject("a este conjunto se le llama la"," \"imagen inversa\" ", "de"," $D$").next_to(text_5a,DOWN)
