...
class ExitenciaLimites(ThreeDScene):
    ...
    def construct(self):
        ...
        ###Texto
        titulo = TextMobject('Exixtencia del límite de funciones de \n $\\mathbb{R}^{n}\\rightarrow\\mathbb{R}^{m}$')
        ...
        definicion_3 = TextMobject('''si $\\forall$ $\\epsilon > 0$ $\\exists$ $\\delta>0$ tal que si $\\Vert  \\vec{x} - \\vec{x}_0 \\Vert  < \\delta$ y $\\vec{x} \\in A \\setminus  \{\\vec{x}_0\}$, ''')
        definicion_4 = TextMobject('''entonces $\\Vert f(\\vec{x}) - \\vec{l} \\Vert < \\epsilon$''')
        ...
        t_14 = TextMobject('''¿Puedes encontrar de forma analítica la \n
...
class Divergencia_A_Infinito(ThreeDScene):
    ...
    def construct(self):
        ...
        definicion_3 = TextMobject('''$\\exists$ $\\delta >0$ tal que si $|t-t_{0}| < \\delta$ ''')
...
class Limites2 (ThreeDScene):
    def construct(self):
        titulo2=TextMobject('''Limite de funciones al infinito de \n
                                $\\mathbb{R}\\rightarrow\\mathbb{R}^{n}$''' )
        texto=TextMobject('''Sea $f:\\mathbb{R}\\rightarrow\\mathbb{R}^{n}$''')
        texto1=TexMobject(r''' \lim_{x \to \infty}f(x)=\vec{L}\leftrightarrow\forall \ \epsilon>0''')
        texto1_1=TextMobject('''$\\exists \\ M\\in\\mathbb{R}$ tal que $x>M \\ \\implies ||f(x)-\\vec{L}||<\\epsilon$''').move_to(
            texto1.get_center()+0.8*DOWN
        )
    ...
    def custom_method(self):
        ...
        texto3=TextMobject('''Los puntos dentro del círculo en el plano\n
                                yz están a una distancia 0.8 del $\\vec{0}$\n
                                del plano yz.''')
        texto3.to_corner(UL)
        texto4=TextMobject('''Veamos que hay un punto en x desde el cual\n
                              f(x) esta a una distancia menor de $\\epsilon$\n
                              del 0 del plano yz.''')
        texto4.to_corner(UL)
        texto4_1=TextMobject('''Es posible hacer lo mismo con cualquier\n
                                $\\epsilon>0$''')
        texto4_1.to_corner(UL)
        texto5=TextMobject('''Por lo cual notaremos que la función tiene \n
                                límite cuando $x\\rightarrow \\infty^{+}$ y es (0,0)''')
        texto5.to_corner(UL)
        texto6=TextMobject('''¿Se te ocurre como modificar la definición \n
        ...
        plano = Rectangle(height=2, width=3,fill_color=YELLOW_C, 
        fill_opacity=0.3)
        ...
        self.wait(2)
        self.play(FadeOut(texto5),FadeOut(circulo1),FadeOut(axes1),
                    FadeOut(surface2))
...
class Limite4_1 (ThreeDScene):
    ...
    ###Animacion
        ...
        self.play(FadeOut(text7),FadeOut(text6),FadeOut(axes),
                    FadeOut(cilindro1),FadeOut(superficie),FadeOut(linea_1),
                    FadeOut(circulo1))        
...
class LimiteR2aR2(Scene):
    ...
    def construct(self):
        ...
        text2_1=TextMobject('''En particular en (0,0) el límite es (0,0)''').to_corner(UL)
        ...
        text5=TextMobject('''Podemos encontrar una bola, tal que la imagen de los\n
                            puntos dentro de la bola'''   ).to_edge(UP)

        text6=TextMobject('''Esten a una distancia menor a''',''' $\\delta$''').to_edge(UP)
        text6[1].set_color(PINK)
        text7=TextMobject('''Es posible hacer lo anterior con cualquier $\\delta$''').to_edge(UP)
        ...
        text8_2=TexMobject(r"\forall\epsilon>0\ \exists\delta>0 \ tq \ si ||\vec{x}-\vec{x_0}||<\delta")
        text8_3=TexMobject(r"(\vec{x}\in A-\lbrace\vec{x_0}\rbrace) \ ||f(\vec{x})-\vec{l}||<\epsilon").move_to(1.5*DOWN)
        ...
        campo_2 = VectorField(campo2, x_min=-8,x_max=8,y_min=-6,y_max=6,
            delta_x=0.25, delta_y=0.25,length_func= lambda norm: 0.25 * sigmoid(norm) )
       ...
       # Dibujamos el campo vectorial 
        ...
        self.wait()
        self.play(FadeOut(campo_2),FadeOut(bola1_1),FadeOut(bola_1),
                    FadeOut(axes),FadeOut(text7),FadeOut(delta1))
        self.play(Write(text8))
    ...
    def construct(self):
       ...
       # Conjunto abstracto en R^n
        ...
        for i in range(-2,2):
            ...
            yellow_dots.append(Dot(point = self.coords_to_point((PI/2)+i*2*PI,1), radius = 0.1, color = YELLOW))
...
class ExtremeValue(GraphScene, Scene):
    ...
    def construct(self):
        ...
        tex1 = TextMobject("Sea", " $f : A \subset \mathbb{R}^{n} \\rightarrow \mathbb{R}$", " continua en $A$ tal que $B \subset A$.").shift(1.5*UP)
        tex2 = TextMobject("Si  $B \\neq \emptyset$,", " cerrado y acotado, entonces $\\exists$", " $ \hat{x_{1}}, \hat{x_{2}} \in B$").next_to(tex1, DOWN)
        tex3 = TextMobject("tales que", " $f(\hat{x_{1}}) \leq f(\hat{x}) \leq f(\hat{x_{2}})$", " $\\forall \hat{x} \in B$.", " Es decir,").next_to(tex2, DOWN)
        tex4 = TextMobject("\\textit{f} alcanza un valor", " máximo", " y un valor", " mínimo", " sobre \\textit{B}.").next_to(tex3, DOWN)
        ...
        pto2 = TextMobject("De la gráfica, se observa que los puntos críticos en $I$ son: ").next_to(pto1, DOWN)
        pto3 = TextMobject("$(0,0)$, $(-1,-11)$, $(3,-27)$ y $(5,25)$.").next_to(pto2,DOWN)
        pto4 = TextMobject("Con lo cual, los valores extremos se alcanzan cuando").next_to(pto3, DOWN)
...
class Superficie(ThreeDScene):
    ...
    def construct(self):
        ...
        nota3 = TextMobject("tal que T es el triángulo con vértices en ").next_to(nota2, DOWN)
        nota4 = TextMobject("(-1,-2,0), (0,1,0), (2,-2,0).").next_to(nota3, DOWN)
        notas = VGroup(ejm2, nota1, nota2, nota3, nota4)
        nota5 = TextMobject("""Queremos verificar que $f$ alcanza un \n
                valor mínimo y un máximo en el triángulo $T$. """).shift(1.5*UP)
        nota6 = TextMobject(""" Como $f$ tiene un único punto crítico en (0,0) \n
                procedemos a encontrar los valores mínimos y máximos  \n
                que $f$ alcanza en la frontera de $T$. """).next_to(nota5, DOWN)
        notas_1 =VGroup(nota5,nota6)

        nota7 = TextMobject("""Encuentra los puntos críticos y verifica que \n
                el valor máximo es $f(\\frac{6}{5}, -\\frac{4}{5})= \\frac{9}{5}$, mientras \n
                que el mínimo corresponde a  $f(0,-2)= 1 $""").scale(0.7).to_corner(DL)
        ...
        sabanita = ParametricSurface(
            lambda u, v: np.array([
                ...
                v,
                u**2-v**2+5
            ]),v_min=-3,v_max=3,u_min=-3,u_max=3,checkerboard_colors=[PURPLE_D, PURPLE_E],
            resolution=(15, 32))
...
#        min4_label = TexMobject(r"$(f(0,1),4)$").next_to(min4, 0.5*LEFT).scale(0.7)
        ...
        minimos = Group (min1, min2, min3, min4, linea_min1, linea_min2, linea_min3,
        linea_min4)
        #Animando texto
        ...
        self.play(ShowCreation(triangulo), ShowCreation(verts), ShowCreation(sabanita),
        ShowCreation(extremos), ShowCreation(minimos), run_time = 12)
        self.wait(3)
        self.play(FadeOut(minimos), FadeOut(extremos), FadeOut(sabanita), FadeOut(verts), FadeOut(triangulo), FadeOut(axes))
        self.wait(2) 
...
class TeoValorIntermedio_1(Scene):
    def construct(self):
        ...
        text_1_1=TextMobject('''Sea $f:A\\subset\\mathbb{R}^{n}\\rightarrow\\mathbb{R}$ ''','''continua''',''' en A''',''' conexo.''').shift(UP*1.8)
        text_1_2 = TextMobject('''$\\vec{x}_1,\\vec{x}_2\\in B$ tales que''').next_to(text_1_1,DOWN)
        text_1_3 = TextMobject('''$f(\\vec{x}_1)<f(\\vec{x}_2)$.''').next_to(text_1_2,DOWN)
        text_1_4 = TextMobject('''Si $c\\in\\mathbb{R}$ es tal que $f(\\vec{x}_1)<c<f(\\vec{x}_2)$,''').next_to(text_1_3,DOWN)
        text_1_5 = TextMobject('''entonces existe $\\vec{x}_c$ tal que $f(\\vec{x}_c)=c$ ''').next_to(text_1_4,DOWN)
...
class TeoValorIntermedio_2(GraphScene):
    ...
    def construct(self):
        ...
        text6=TextMobject('''$\\Vec{x}$ puede no ser el único elemento\n
                                que satisface que $f(\\Vec{x})=1$''').move_to(3.3*UP)
        ...
        vec_x2_label=TexMobject(r"f(\Vec{x}_2)").next_to(vec_x2,RIGHT,buff=0.2).set_color(PINK)
        ...
        x_pp=0.8961
        label_x_p=TexMobject(r"f(\vec{x})").next_to(vec_x_p,RIGHT,buff=0.1).set_color(GREEN)
        fx_pp=np.exp(x_pp*x_pp)*np.sin(10*x_pp)
        vec_x_pp=Dot(color=GREEN).move_to(self.coords_to_point(x_pp,fx_pp))
        #label_x_pp=TexMobject(r"f(\vec{x})").next_to(vec_x_pp).set_color(GREEN)
        ...
        #self.wait(7)
        self.play(ReplacementTransform(text4.group,text6.group))
...
class TeoValorIntermedio_3 (ThreeDScene):
    ...
    def construct(self):
        ...
        text2=TextMobject('''Sea $f(x,y)=x^{2}+y^{2}$''')
        for i in range(3,9):
        ...
        text3=TextMobject('''Y sea $\\Vec{x}_1=(0,-2),\\Vec{x}_2=(3,0)$\n''',
                                    '''y $c=5$''')
        text3.to_edge(DOWN)
        text3.bg = SurroundingRectangle(text3,color=WHITE,fill_color=BLACK,fill_opacity=1)
        text3.group = VGroup(text3.bg,text3)
        ...
        text3[0][17].set_color(RED)
        text3[0][15].set_color(RED)
        text3[0][16].set_color(RED)
        text4=TextMobject('''$f(\\Vec{x}_1)$''','''$=4<5<9=$''','''$f(\\Vec{x}_2)$''')
        text4.to_edge(DOWN)
        text4.bg = SurroundingRectangle(text4,color=WHITE,fill_color=BLACK,fill_opacity=1)
        text4.group = VGroup(text4.bg,text4)
        text4[0].set_color(PURPLE_B)
        text4[2].set_color(RED)
        text5=TextMobject('''Si tomamos $\\vec{x}=(x,y)$ tal que $||\\vec{x}||^{2}=5$, \\\\''','''se cumple que $f(\\vec{x})=5$.''')
        text5.to_edge(DOWN)
        ...
        text6_1.group = VGroup(text6_1.bg,text6_1)
        text7 = TextMobject('''Si $f:D\\subseteq \\mathbb{R}^n\\rightarrow\\mathbb{R}^m$ y $D$ es conexo, \\\\
                            ¿Qué puedes decir de la imagen de $f$? \\\\''',
                            '''¿Es conexa?''')
        for i in range(15,21):
            text7[0][i].set_color(ORANGE)
        for i in range(3,9):
            text7[1][i].set_color(ORANGE)
        ...
        #f_1=Sphere(radius=0.04, color=PURPLE_B).move_to([x1,y1,x1**2+y1**2-2]).set_color(PURPLE_C).set_shade_in_3d(True) 
        ...
        #f_2=Sphere(radius=0.04,color=RED).move_to([x2,y2,x2**2+y2**2-2]).set_color(RED).set_shade_in_3d(True) 
        f_2_label=TexMobject(r"\vec{x}_2").rotate(PI/2,axis=RIGHT).move_to(f_2.get_center()+0.4*DOWN+0.6*RIGHT ).set_color(RED).scale(1.5)
        #Tambien se puede cambiar el radio de la bola y su posición
        bola=Circle(radius=(5**(1/2)),color=ORANGE).move_to([0,0,5])
        ...
        self.add_fixed_in_frame_mobjects(text3[0],text3.bg)
        ...
        self.play(FadeOut(text6_1.group),FadeOut(superficie),FadeOut(axes),FadeOut(G1),FadeOut(bola))
